...
### Basic of the EM Algorithm
  ...
  - E-Step (Extraction step): Given the current parameter estimates, calculate the expected values of the latent variables or unobsered data.
  ...
  - Iteration: Repeate steps 2 and 3 until convergence, which is typically determined by a predefined convergence criterion. 
...
*Goal is to find the parameter values that maximize the likelihood; the estimation "crawls along" with estimation to find "the peak."*
  ...
  - Stopping criteria is neededd or it will iterate forever and you wil never get an output file.
